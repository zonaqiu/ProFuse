...
def _generate_fusion_image(model, strategy_type, img):
	a = 0.5
	# encoder
	x1,x2,x3,x4= model.encoder(img)
	# decoder
	img_fusion = model.decoder(x1,x2,x3,x4)
	return img_fusion[0]


def run_demo(model, path, output_path_root, index, fusion_type, network_type, strategy_type, ssim_weight_str, mode):

	img = utils.get_test_images(path, height=None, width=None, mode=mode)
	...
	img_fusion = _generate_fusion_image(model, strategy_type, img)
	############################ multi outputs ##############################################
...
def main():
	...
	test_path = "/data/disk_c/zona/data/C_TEST_psnr/"
	...
	output_path = '/data/disk_c/zona/U-net/test12/output/test12-psnr/'
	...
	with torch.no_grad():
		...
		for i in range(26):
			index = i + 1
			path = test_path + str(index) + houzhui
			run_demo(model, path, output_path, index, fusion_type, network_type, strategy_type, ssim_weight_str, mode)
	print('Done......')
