...
def main():
	...
	train(0 , original_imgs_path)

def train(i, original_imgs_path):
	writer = SummaryWriter('/data/data_c/zona/U-net/runs/test121e{}'.format(i))
	...
	optimizer = Adam(unet_model.parameters(),args.lr)
	...
	for e in tbar:
		...
		for batch in range(batches):
			...
			x1,x2,x3,x4 = unet_model.encoder(img)
			outputs = unet_model.decoder(x1,x2,x3,x4)
			# resolution loss
			...
			#log_interval = 5 ，20*5个batch之后输出损失函数的信息
			if (batch + 1) % args.log_interval == 0:
				...
				writer.add_scalar('ssim_loss',all_ssim_loss/args.log_interval,count)
				writer.add_scalar('total_loss', (args.ssim_weight[i] * all_ssim_loss + all_pixel_loss) / args.log_interval,count)
			...
			if (batch + 1) % (200 * args.log_interval) == 0:
				img_dir = '/data/data_c/zona/U-net/test12/output/'
				img_name = args.ssim_path[i] + '/' + "Epoch_" + str(e) + "_iters_" + str(count) +  "_" + args.ssim_path[i] + ".png"
